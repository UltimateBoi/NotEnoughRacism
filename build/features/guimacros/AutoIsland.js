...
const autoIsland = () => {
    if (config.autoIslandType === 0) {
        if (doIGoBack) {
            new Thread(() => {
                Thread.sleep(5000)
                ChatLib.chat(PREFIX + "&cIsland restart detected while afk. Sending back to island")
                ChatLib.command("is")
                doIGoBack = false
            }).start()
        }
        ...
        if (doIGoBack) {
            new Thread(() => {
                Thread.sleep(5000)
                ChatLib.chat(PREFIX + "&cThe Private Island you were on is restarting, sending you back to " + config.autoIslandIGN + "'s Private Island")
